...
def prediction_heat_map(model, img, width, height, stride):
    ...
    height_range = ((img_height / height) * (height / stride)) - (height / stride) + 1
    width_range = ((img_width / width) * (width / stride)) - (width / stride) + 1
    for i in range(width_range):
        for j in range(height_range):
            img_heat_map = prediction_heat_map_helper(
                i, j, width, height, stride, img, model, False
            )
...
def prediction_heat_map_helper(i, j, width, height, stride, img, model, show=False):
    width_start = i * stride
    height_start = j * stride
    x = np.copy(img)
    x[
        :, width_start : width_start + width, height_start : height_start + height, :
    ].fill(fill_color)
...
def create_heatmaps(log_path, radar_product, epoch=""):
    ...
    for i in range(len(filenames)):
        ...
        heat_maps.append(prediction_heat_map(model, img, 48, 48, 16))
        heat_maps.append(prediction_heat_map(model, img, 60, 60, 30))
        heat_maps.append(prediction_heat_map(model, img, 80, 80, 20))
...
def main():
    ...
    create_heatmaps(log_path=log_path, radar_product=radar_product, epoch=str(1700))
